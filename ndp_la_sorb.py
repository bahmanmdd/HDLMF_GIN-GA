"""
   ...
   Description: generate an instance of D-NDP with lane additions and solve using SORB algorithm (Wang et al. 2013)
"""
...
def params_for_test():
    ...
    return case[0], data_dir, variation, ods_dict[case[0]], net_dict[case[0]]
...
def algorithm_SORB(node_features, edge_features, variation):
    ...
    while not converged:
        ...
        rp_dvs, rp_ttt, rp_cap, rp_time = rp_model_pwl1_solve(
...
# piecewise linear with 1 additional lane (binary)
def rp_model_pwl1_solve(node_features, edge_features, variation, params, dvs_rp_all, t_remain):

    """
    The rp_model_pwl1_solve function solves the problem with SORB using a piecewise linear approximation of the link
    travel time function with 1 additional lane per link.
    ...
    lanes = range(lanes_max + 1)
    nodes = np.unique([list(edge) for edge in links])
    capacity = {(a, b): cap/scale for (a, b), cap in edge_features['capacity'].items()}
    capacity_extra = {(i, j): capacity[i, j]/lanes_n for (i, j) in links}
    lane_costs = {(i, j): (fftt[i, j] * capacity[i, j]) for (i, j) in links}

    budget = budget_portion * np.sum([lane_costs[i, j] for (i, j) in links])
    ...
    init_dvs = {(i, j): 0 for (i, j) in links}
    coef_0 = {(i, j): (fftt[i, j] * alpha[i, j])/(capacity[i, j] ** beta[i, j]) for (i, j) in links}
    coef_1 = {(i, j): (fftt[i, j] * alpha[i, j])/((capacity[i, j]+capacity_extra[i, j]) ** beta[i, j]) for (i, j) in links}
    ...
    # decision variables
    dvs_upper = {(i, j): model_rp.binary_var() for (i, j) in links}
    ...
    # budget constraint
    model_rp.add_constraint(sum(dvs_upper[i, j] * lane_costs[(i, j)] for (i, j) in links) <= budget)
    ...
    if dvs_rp_all:
        for solution in dvs_rp_all:
            model_rp.add_constraint(sum(
                    (1 - solution[i, j]) * dvs_upper[i, j] +
                    (1 - dvs_upper[i, j]) * solution[i, j]
                    for (i, j) in links) >= 1)

    model_rp.minimize(sum(fftt[i, j] * sum(dvs_lower[i, j, s] for s in destinations) +
                          (coef_0[i, j] * (1 - dvs_upper[i, j]) + coef_1[i, j] * dvs_upper[i, j]) *
    ...
    try:
        ...
        rp_dvs = {(i, j): round(model_rp.solution.get_value(dvs_upper[i, j])) for (i, j) in links}
        rp_cap = {(i, j): (capacity[i, j] + rp_dvs[i, j] * capacity_extra[i, j]) * scale for (i, j) in links}
        rp_lf = {}
        for (i, j) in links:
